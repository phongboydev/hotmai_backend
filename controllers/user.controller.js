...
const addUser = async (req, res) => {
  ...
  try {
    ...
    const newUser = await User.create({username:user.username,password:user.password,role:user.role,balance:user.balance});
    if(newUser){
      res.status(200).json({
        isSuccess: true,
        data: newUser,
...
const register = async (req, res) => {
  ...
  try {
    ...
    const hashPassword = bcrypt.hashSync(user.password, salt);
    user.password = hashPassword;
    const newUser = await User.create({username:user.username,password:user.password,role:'client',balance:0});
  ...
  if (user) {
    ...
    if (isAuth) {
      const token = jwt.sign(
        { username: user.username, role: user.role,balance:user.balance,id:user.id },
        "Dai-Phong-1807",
        { expiresIn: 60 * 60 }
      );
